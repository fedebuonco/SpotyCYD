...
public:
  ...
  void displayTrackProgress(long progress, long duration) {
    ...
    tft.fillRect(20 + barXWidth, progressStartY + 1,
                 (screenWidth - 20) - (20 + barXWidth), 18, TFT_BLACK);
  }

  void printCurrentlyPlayingToScreen(CurrentlyPlaying currentlyPlaying) {
    // Clear the text
    int textStartY = 150 + 30;
    tft.fillRect(0, textStartY, screenWidth, screenHeight - textStartY,
                 TFT_BLACK);

    tft.drawCentreString(currentlyPlaying.trackName, screenCenterX, textStartY,
                         2);
    tft.drawCentreString(currentlyPlaying.artists[0].artistName, screenCenterX,
                         textStartY + 18, 2);
    tft.drawCentreString(currentlyPlaying.albumName, screenCenterX,
                         textStartY + 36, 2);
  }

  void checkForInput() {
    if (millis() > touchScreenCoolDownTime && handleTouched()) {
      drawTouchButtons(previousTrackStatus, nextTrackStatus, playStatus);
      if (previousTrackStatus) {
        spotify_display->previousTrack();
      } else if (nextTrackStatus) {
        spotify_display->nextTrack();
      } else if (playStatus) {
        // Get the current play status and act on it
        if (playing) {
      ...
      drawTouchButtons(false, false, false);
      requestDueTime = 0; // Some button has been pressed and acted on, it
                          // surely impacts the status so force a refresh
      touchScreenCoolDownTime =
          millis() + touchScreenCoolDownInterval; // Cool the touch off
    }
  }
  ...
  boolean processImageInfo(CurrentlyPlaying currentlyPlaying) {
    SpotifyImage currentlyPlayingMedImage =
        currentlyPlaying.albumImages[currentlyPlaying.numImages - 2];
    if (!albumDisplayed || !isSameAlbum(currentlyPlayingMedImage.url)) {
      ...
      setImageHeight(
          currentlyPlayingMedImage.height /
          2); // medium image is 300, we are going to scale it to half
  ...
  void markDisplayAsTagRead() {
    ...
    tft.drawRect(imagePosition + 2, 2, imageWidth - 4, imageHeight - 4,
                 TFT_RED);
  }
...
private:
  bool playing = false;
  unsigned long touchScreenCoolDownInterval =
      200; // How long after a touch press do we accept another (0.2 seconds).
           // There is also an APi request inbetween
  ...
  int displayImageUsingFile(char *albumArtUrl) {
    ...
    // Spotify uses a different cert for the Image server, so we need to swap to
    // that for the call
  ...
  int drawImagefromFile(const char *imageFileUri) {
    ...
    lTime = millis();
    jpeg.open((const char *)imageFileUri, myOpen, myClose, myRead, mySeek,
              JPEGDraw);
    jpeg.setPixelType(1);
    int imagePosition = screenCenterX - (imageWidth / 2);
  ...
  void drawTouchButtons(bool backStatus, bool forwardStatus, bool playStatus) {

    int buttonCenterY = 75;
    int leftButtonCenterX = 40;
    int rightButtonCenterX = screenWidth - leftButtonCenterX;
    int playButtonCenterX = screenWidth / 2;
    ...
    tft.fillTriangle(leftButtonCenterX - 4, buttonCenterY,
                     leftButtonCenterX + 6, buttonCenterY - 10,
                     leftButtonCenterX + 6, buttonCenterY + 10, TFT_WHITE);
    ...
    tft.fillTriangle(rightButtonCenterX + 4, buttonCenterY,
                     rightButtonCenterX - 6, buttonCenterY - 10,
                     rightButtonCenterX - 6, buttonCenterY + 10, TFT_WHITE);
    tft.drawRect(rightButtonCenterX + 6, buttonCenterY - 10, 2, 20, TFT_WHITE);

    // Draw Play/Pause Button with visual enhancements
    tft.fillRoundRect(playButtonCenterX - 20, buttonCenterY - 20, 40, 40, 10,
                      TFT_WHITE);
    tft.drawRoundRect(playButtonCenterX - 20, buttonCenterY - 20, 40, 40, 10,
                      TFT_WHITE);

    if (playStatus) {
      tft.fillRect(playButtonCenterX - 6, buttonCenterY - 10, 5, 20, TFT_WHITE);
      tft.fillRect(playButtonCenterX + 2, buttonCenterY - 10, 5, 20, TFT_WHITE);
    } else {
      tft.fillTriangle(playButtonCenterX - 6, buttonCenterY - 12,
                       playButtonCenterX + 10, buttonCenterY,
                       playButtonCenterX - 6, buttonCenterY + 12, TFT_WHITE);
    }
  }
};
